...
export class AuthService {
  ...
  async signUp(signUpDto: SignUpDto): Promise<{ accessToken: string, refreshToken: string }> {
    ...
    const accessToken = this.generateToken(String(user._id), '15m'); // Set expiration time to 15 minutes
    const refreshToken = await this.refreshTokenService.generateRefreshToken(String(user._id));
  ...
  async refreshTokens(refreshToken: string): Promise<{ accessToken: string, refreshToken: string }> {
    ...
    const accessToken = this.generateToken(String(userId), '15m'); // Set expiration time to 15 minutes
    return { accessToken, refreshToken };
  }
